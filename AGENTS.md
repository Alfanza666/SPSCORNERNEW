...
- **🚨 AI Thinking Tag Rule: Respons model dengan mode thinking (seperti qwen atau deepseek) bisa mengandung tag `<think>...</think>` yang terpotong tanpa penutup jika token habis. Helper `stripAiWrapper()` di `src/services/griphub.js` WAJIB menangani tag lengkap maupun tag terpotong, serta mengekstrak objek JSON `{...}`.** Lihat v6.0.20 di `changelog.txt`.
- **🚨 AI Provider Order: Griphub Router (`gpt-5.6-luna`) adalah provider AI UTAMA (kecepatan stabil 2-4s, native vision, format JSON konsisten). Groq bertindak sebagai cadangan otomatis jika Griphub gagal/timeout.** Lihat v6.0.20 di `changelog.txt`.
- **🚨 AI JSON Mode Rule: DILARANG mengirim `response_format: { type: "json_object" }` saat memanggil model yang tidak mendukung strict mode (seperti qwen di Groq). Andalkan prompt + parsing JSON yang toleran terhadap markdown fence.** Lihat v6.0.18 di `changelog.txt`.
- **🚨 AI Fallback Model Rule: Saat fallback lintas provider, model WAJIB disesuaikan dengan model provider tujuan (`GRIPHUB_VISION_MODEL`/`GRIPHUB_MODEL` untuk Griphub, `GROQ_VISION_MODEL`/`GROQ_MODEL` untuk Groq). DILARANG meneruskan model antar provider tanpa mapping.** Lihat v6.0.18 di `changelog.txt`.
