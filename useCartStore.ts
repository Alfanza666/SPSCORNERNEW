...
Sistem SPS Corner`);
  ...
  const handleCheckStatus = async (item: TransactionItem) => {
    try {
      ...
      if (data.success) {
        if (data.itemStatus === 'delivered') toast.success('Pesanan berhasil! SN: ' + (data.sn || 'Tersedia'));
        else if (data.itemStatus === 'failed') toast.error('Pesanan gagal: ' + data.message);
        else toast.success('Pesanan masih diproses...');
        fetchHistory(); // Refresh to get updated SN
      }
    } catch (error: any) {
                ...
                <div className="p-4 sm:p-5">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {item.metadata?.is_digital && (
                                <div className="flex items-center gap-2">
                                  ...
                                  {item.status !== 'delivered' && item.status !== 'failed' && (tx.status === 'paid' || tx.status === 'success') && (
                                    <button
                                      onClick={() => handleCheckStatus(item)}
                                      disabled={checkingItem === item.id}
                                      className="text-[8px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-widest bg-zinc-200 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-300 disabled:opacity-50 transition-colors"
                                    >
                                      {checkingItem === item.id ? 'Mengecek...' : 'Cek Status'}
                                    </button>
                                  )}
                                </div>
                              )}
                  ...
                  <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-zinc-100 dark:border-zinc-800 border-dashed flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {tx.status === 'success' && (
                        <>
                          {tx.transaction_items.some(item => item.products?.category?.toLowerCase() === 'sariroti' || item.products?.name?.toLowerCase().includes('sariroti')) && (
                            <button 
