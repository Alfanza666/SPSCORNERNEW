...
export default function DashboardLayout() {
  ...
  const canAccessPortal = isEmployeeNik(user?.nik);
  const dashboardHomePath = isAdmin ? '/dashboard/admin' : '/dashboard/seller';
  const isDashboardHomeActive = location.pathname === dashboardHomePath;
  ...
  const renderNavItems = () => {
    ...
    const adminGroups = [
      ...
      },
      {
        ...
        items: [
          ...
          { to: "/dashboard/admin/employees", icon: Users, label: "Database Karyawan" }
        ]
      },
      {
        label: "Logistik & Stok",
    ...
    const sellerItems = [
      ...
      { to: "/dashboard/seller/stock-trace", icon: Search, label: "Riwayat Stok" },
    ...
    return (
      <>
        {isAdmin && adminGroups.map((group, gIdx) => {
          ...
          const groupHasActiveItem = group.items.some(item => location.pathname === item.to || location.pathname.startsWith(`${item.to}/`));
          ...
          return (
            <NavGroup
              ...
              label={group.label}
              icon={group.icon}
              defaultOpen={Boolean(menuSearchQuery) || groupHasActiveItem}
            >
  ...
  return (
    <div className="min-h-screen bg-[#e8ebf0] dark:bg-zinc-950 flex overflow-hidden transition-colors duration-300">
            ...
            >
              <Dialog.Panel className="relative mr-10 flex w-full max-w-[min(320px,calc(100vw-2.5rem))] flex-1 sm:mr-16">
                <div className="flex h-full w-full flex-col bg-white dark:bg-zinc-900 shadow-2xl dark:shadow-black">
                  <div className="flex h-[4.5rem] min-h-[4.5rem] shrink-0 items-center justify-between border-b border-zinc-100 px-5 dark:border-zinc-800">
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar p-3 space-y-5">
                    <div className="px-1">
                      ...
                      <div className="mb-2 px-3 py-2 text-[10px] font-black uppercase tracking-[0.24em] text-zinc-400 dark:text-zinc-500">
                        Menu Utama
                      </div>
                  ...
                  <div className="border-t border-zinc-100 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
                    <div className="mb-3 flex items-center gap-3 rounded-[1.35rem] border border-white bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-800/50">
                    ...
                    {notifications.filter(n => !n.isRead).length === 0 ? (
                      ...
                      notifications.filter(n => !n.isRead).map((notif) => (
                        <Menu.Item key={notif.id}>
                          {({ active }) => (
                            <div
                              className={`m-2 sm:m-3 p-3 sm:p-5 rounded-xl sm:rounded-2xl cursor-pointer group transition-all shadow-sm ${active ? 'ring-2 ring-blue-500 ring-offset-2 ring-offset-white dark:ring-offset-zinc-900' : ''
                            ...
                            >
                              <div className="flex gap-2 sm:gap-4">
                                <div className={`w-8 h-8 sm:w-12 sm:h-12 rounded-lg sm:rounded-2xl flex items-center justify-center flex-shrink-0 clay-icon group-hover:scale-105 transition-transform shadow-sm ${notif.type === 'transaction' ? 'bg-white dark:bg-zinc-800 text-blue-600 dark:text-blue-400' :
                                  ...
                                  {notif.type === 'transaction' ? <Receipt className="w-4 h-4 sm:w-6 sm:h-6" /> :
                                    notif.type === 'withdrawal' ? <CreditCard className="w-4 h-4 sm:w-6 sm:h-6" /> :
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-[9px] sm:text-[10px] text-zinc-400 dark:text-zinc-500 font-bold uppercase tracking-widest mt-1 sm:mt-3">
      ...
      <nav
        aria-label="Navigasi cepat dashboard"
        className="fixed inset-x-0 bottom-0 z-40 border-t border-white/20 bg-white/92 px-2 pb-[calc(env(safe-area-inset-bottom)+0.55rem)] pt-2 shadow-[0_-18px_45px_rgba(15,23,42,0.16)] backdrop-blur-2xl dark:border-zinc-800/80 dark:bg-zinc-950/92 lg:hidden"
      >
        <div className={`mx-auto grid max-w-md ${canAccessPortal ? 'grid-cols-4' : 'grid-cols-3'} gap-1.5`}>
          <button
            type="button"
            aria-current={isDashboardHomeActive ? 'page' : undefined}
            onClick={() => navigate(dashboardHomePath)}
            className={`flex min-w-0 flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2.5 text-[10px] font-black transition-all ${isDashboardHomeActive
          ...
          <button
            type="button"
            onClick={() => setIsSidebarOpen(true)}
            className="flex min-w-0 flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2.5 text-[10px] font-black text-zinc-500 transition-all hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-white"
          >
          ...
          <button
            type="button"
            onClick={() => navigate('/kiosk')}
            className="flex min-w-0 flex-col items-center justify-center gap-1 rounded-2xl bg-gradient-to-br from-amber-300 to-orange-400 px-2 py-2.5 text-[10px] font-black text-amber-950 shadow-lg shadow-orange-500/20 transition-all active:scale-[0.98]"
          >
