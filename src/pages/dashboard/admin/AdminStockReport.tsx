...
import { BarChart3, Download, Search, Loader2, FileSpreadsheet, AlertCircle } from 'lucide-react';
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="bg-white dark:bg-zinc-800/50 rounded-2xl border border-zinc-200 dark:border-zinc-700 overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center px-4">
            ...
            <p className="text-xs text-zinc-400 mt-1 max-w-md">{error}</p>
            <button onClick={fetchReport} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-xl text-sm hover:bg-blue-700 transition-colors">Coba Lagi</button>
          </div>
        ) : filtered.length === 0 ? (
