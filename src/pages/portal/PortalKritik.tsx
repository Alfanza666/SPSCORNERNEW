...
import { 
  MessageSquare, AlertTriangle, Shield, Send, Clock, CheckCircle, Loader2, FileText, ChevronRight, ChevronLeft, Sparkles
} from 'lucide-react';
...
export default function PortalKritik() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      appToast.success('Terkirim!', 'Kritik/saran Anda telah dikirim.');
      setFormData({ type: 'saran', title: '', description: '' });
      fetchMyFeedback();
    } catch (error) {
      console.error('Error submitting feedback:', error);
      appToast.error('Gagal Kirim', 'Terjadi kesalahan saat mengirim kritik/saran.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'kritik': return <AlertTriangle className="w-4 h-4 md:w-5 md:h-5" />;
      case 'saran': return <MessageSquare className="w-4 h-4 md:w-5 md:h-5" />;
      ...
      default: return <MessageSquare className="w-4 h-4 md:w-5 md:h-5" />;
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 font-medium mt-1">Untuk meeting bipartit</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center gap-3 mb-5 md:mb-6">
            ...
            <h3 className="font-bold text-zinc-900 dark:text-white text-base md:text-lg">Kirim Kritik/Saran</h3>
          </div>

          <form onSubmit={handleSubmit} className="space-y-5 md:space-y-6">
            <div>
              <label className="block text-xs md:text-sm font-bold text-zinc-500 dark:text-zinc-400 mb-3 uppercase tracking-wider">Jenis</label>
            ...
            <div>
              <label className="block text-xs md:text-sm font-bold text-zinc-500 dark:text-zinc-400 mb-2 uppercase tracking-wider">Judul</label>
              <input
                type="text"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                className="w-full p-3 md:p-4 rounded-xl md:rounded-2xl border-2 border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 text-sm md:text-base focus:border-purple-400 focus:ring-0 outline-none transition-all"
            ...
            <div>
              <label className="block text-xs md:text-sm font-bold text-zinc-500 dark:text-zinc-400 mb-2 uppercase tracking-wider">Detail</label>
              <textarea
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                className="w-full p-3 md:p-4 rounded-xl md:rounded-2xl border-2 border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 text-sm md:text-base focus:border-purple-400 focus:ring-0 outline-none h-32 md:h-40 resize-none transition-all"
                placeholder="Jelaskan detail kritik/saran Anda secara lengkap..."
                required
              />
            </div>

            <div className="bg-gradient-to-r from-purple-50 to-violet-50 dark:from-purple-900/30 dark:to-violet-900/20 p-3 md:p-4 rounded-xl md:rounded-2xl border border-purple-100 dark:border-purple-900/30">
              <div className="flex items-center gap-2 mb-2">
                <Sparkles className="w-4 h-4 text-purple-500" />
                <p className="text-xs md:text-sm font-bold text-purple-700 dark:text-purple-300">Catatan</p>
              </div>
              <p className="text-xs md:text-sm text-purple-700 dark:text-purple-300 leading-relaxed">
                Kritik dan saran Anda akan diproses di meeting bipartit berikutnya. Identitas Anda akan dirahasiakan.
              </p>
            </div>
            ...
            <motion.button
              ...
              type="submit"
              disabled={submitting}
              className="w-full py-3 md:py-4 bg-gradient-to-r from-purple-600 to-violet-600 hover:from-purple-700 hover:to-violet-700 text-white font-bold rounded-xl md:rounded-2xl flex items-center justify-center gap-2 md:gap-3 shadow-lg shadow-purple-500/30 transition-all disabled:opacity-50 text-sm md:text-base"
            >
              {submitting ? (
          ...
          {loading ? (
            <div className="flex justify-center py-8 md:py-12">
              <motion.div
                ...
                className="w-8 h-8 md:w-10 md:h-10 border-3 md:border-4 border-purple-500 border-t-transparent rounded-full"
              />
            </div>
          ) : feedbacks.length === 0 ? (
            <div className="text-center py-8 md:py-12 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl md:rounded-2xl">
              <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-zinc-100 dark:bg-zinc-700 flex items-center justify-center mx-auto mb-3 md:mb-4">
                <MessageSquare className="w-8 h-8 md:w-10 md:h-10 text-zinc-400" />
              </div>
              <p className="text-zinc-500 font-bold text-base md:text-lg mb-1">Belum ada riwayat</p>
              <p className="text-xs md:text-sm text-zinc-400">Kirim kritik/saran pertama Anda sekarang</p>
            </div>
          ) : (
                        ...
                        {feedback.response && (
                          <div className="mt-3 md:mt-4 p-3 md:p-4 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/30 dark:to-emerald-900/20 border border-green-100 dark:border-green-900/30 rounded-xl md:rounded-2xl">
                            <p className="text-sm font-bold text-green-700 dark:text-green-400 flex items-center gap-2 mb-2">
                              <CheckCircle className="w-4 h-4 md:w-5 md:h-5" /> Tanggapan Manajemen:
                            </p>
