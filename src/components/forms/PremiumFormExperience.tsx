...
function PremiumField({ field, value, addonOrders, uploading, onChange, onAddonChange, onFileSelect }: PremiumFieldProps) {
  ...
  if (field.type === 'addon_group') return <div className="space-y-3">{field.items?.map(item => {
    ...
    return <div key={item.id} className="flex flex-col gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 sm:flex-row sm:items-center dark:border-zinc-700 dark:bg-zinc-950/50"><div className="min-w-0 flex-1"><p className="font-bold text-zinc-900 dark:text-white">{item.name}</p><p className="mt-1 text-xs text-zinc-500">{item.sizes.join(', ')} · {formatCurrency(item.price)}</p></div><div className="flex items-center gap-2 self-end sm:self-auto"><button type="button" onClick={() => onAddonChange(quantity <= 1 ? addonOrders.filter(order => order.item_id !== item.id) : addonOrders.map(order => order.item_id === item.id ? { ...order, quantity: order.quantity - 1 } : order))} className="flex h-10 w-10 items-center justify-center rounded-xl border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900"><Minus className="h-4 w-4" /></button><span className="w-8 text-center font-bold tabular-nums">{quantity}</span><button type="button" onClick={() => onAddonChange(quantity === 0 ? [...addonOrders, { item_id: item.id, quantity: 1 }] : addonOrders.map(order => order.item_id === item.id ? { ...order, quantity: order.quantity + 1 } : order))} className="flex h-10 w-10 items-center justify-center rounded-xl bg-zinc-950 text-white dark:bg-white dark:text-zinc-950"><Plus className="h-4 w-4" /></button></div></div>;
  })}</div>;
