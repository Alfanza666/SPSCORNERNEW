...
# ── 5. Check & install new dependencies ──
...
if ($localPkgHash -ne $remotePkgHash) {
  Write-Host "⚠️  Dependencies changed — installing..." -ForegroundColor Yellow
...
try {
  ...
  Write-Host "✅ Server is healthy — response: $($response | ConvertTo-Json -Compress)" -ForegroundColor Green
} catch {
